...
"""
    ...
    Данная спецификация описывает API, который должен вызываться Партнёром в рамках процесса работы с ready-to-eat заказами.  В случае, если Партнёр работает в режиме собственной сборки только для самовывоза, данный API должен использоваться только для заказов, у которых выбран тип доставки `pickup` (самовывоз)  При работе с данным API нужно дополнительно передавать HTTP заголовок `Api-Version: 3.0` во все запросы уведомлений. Для аутентификации необходимо к запросу добавить заголовок client-token со значением токена, полученным в разделе “Получение токена”
